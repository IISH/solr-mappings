...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template name="listitem">
        ...
        <xsl:if test="string-length($displayLabel)!=0">
            ...
            <xsl:choose>
                ...
                <xsl:value-of select="$displayLabel"/></dt>
                <xsl:choose>
                    <xsl:when test="$value/dd"><xsl:copy-of select="$value"/></xsl:when>
                    <xsl:otherwise><dd><xsl:value-of select="$value"/></dd></xsl:otherwise>
                </xsl:choose>
        </xsl:if>
                ...
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        ...
                        <dd class="sru index"><xsl:value-of select="$map"/></dd>
                        <dd class="sru relation"><xsl:value-of select="$relation"/></dd>
                        <dd class="sru interfieldoperator"/>
                        <dd class="sru recordSchema"><xsl:value-of select="$recordSchema"/></dd>
                        <dd class="sru maximumRecords"><xsl:value-of select="$maximumRecords"/></dd>
                    </xsl:otherwise>
                </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="imageitem">
        ...
        <dt>images</dt>
            ...
            <dd class="image"><a href="{text()}" target="_blank"><img src="{text()}" height="75"/></a></dd>
            </xsl:for-each>
    </xsl:template>
